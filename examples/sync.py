...
try:
    ...
    if not invoice.payment_details:
        updated_invoice = client.patch_invoice(
    ...
    prolonged_invoice = client.prolongate_invoice(id=invoice.id)
...
# Get invoice history
try:
    invoice_history = client.get_invoices(
    ...
    if invoice_history.total_count > len(invoice_history.invoices):
        ...
        more_invoices = client.get_invoices(
            from_date=invoice_history.invoices[-1].created_at,  # Use last invoice date
            to_date="2025-03-31T23:59:59.999Z",
            limit=10,
        )
...
# Create a wallet
try:
    wallet = client.create_wallet(order_id="wallet-order-123")
    ...
    # Get wallet details
    wallet_details = client.get_wallet(id=wallet.id)
...
try:
    transaction_history = client.get_transactions(
...
#  Using ChiefPayClient (combines REST and WebSocket)
try:
    chief_client = ChiefPayClient(api_key="your_api_key")

    # REST API via chief_client.rest
    rates = chief_client.rest.get_rates()
    print(f"Rates via ChiefPayClient: {len(rates)} rates available")
